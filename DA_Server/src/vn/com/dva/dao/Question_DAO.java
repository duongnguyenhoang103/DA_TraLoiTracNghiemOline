...
public class Question_DAO {
    ...
    Subject_DAO subjDAO = new Subject_DAO();
    ...
    public DefaultTableModel getAllQuestiontoTable() {
        ...
        mode.addColumn("ID");
        mode.addColumn("Tiêu đề câu hỏi");
        mode.addColumn("ID Người tạo");
        // mode.addColumn("Người tạo");
        mode.addColumn("Câu hỏi thuộc môn");
...
//               String nameUser = user.getUserName();
                Long IdSubj = q.getSubjectID();
                Subject subj = subjDAO.getSubjectByID(IdSubj);
                String nameSubj = subj.getSubjectName();

                String level = "Khó";
                if (q.getLevelID() == 1) {
                    level = "Dễ";
                } else if (q.getLevelID() == 2) {
                    level = "Trung Bình";
                }

                mode.addRow(new Object[]{q.getQuestionID(), q.getQuestionTitle(), q.getUserID(), nameSubj, level, q.getContent(), q.getDateCreate()});
            }
        }
        return mode;
    }
